...
public class DemoController {
	...
	public String home(@RequestParam(value = "queue", required = false, defaultValue = "demo") String queue, Model model) {
		model.addAttribute("queue", queue);
		return "home";
	}

	@RequestMapping("/data")
	public Object data(@RequestParam(value = "queue", required = false, defaultValue = "demo") String queue)
			throws Exception {

		UriComponents uriComponents = UriComponentsBuilder.newInstance().scheme("http")
				.host(rabbitProperties.getHost())
				.port(15672)
				.pathSegment("api", "queues", rabbitProperties.getVirtualHost().replaceAll("/", "%2F"), queue)
		...
		ResponseEntity<String> exchange = template.exchange(new URI(uriComponents.toUriString()), HttpMethod.GET,
				new HttpEntity<String>(createHeaders(rabbitProperties.getUsername(), rabbitProperties.getPassword())),
				String.class);
